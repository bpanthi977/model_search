...
class DatasetConfig:
    ...
    subset: int = field(default=-1, metadata={"help": "Only use first N samples for training. If subset = -1 use all dataset. It is helpful during initial model selection, to finda a model that can memorize the dataset."})
...
class TrainConfig:
    ...
    evaluation_metric: str = field(default='val_loss', metadata={'help': 'The evaluation metric to choose the best checkpoint and tuning. [val_loss]'})
    ...
    def __post_init__(self):
        ...
        check_member('evaluation_metric', self.evaluation_metric, ['val_loss'])
